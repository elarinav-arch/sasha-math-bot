...
- GitHub Actions запускается по крону в 11:00 / 14:00 / 16:00 UTC (14:00 / 17:00 / 19:00
  по Кипру) и ждёт ответов до часа (long polling). Вне окон бот не отвечает — время
  следующей тренировки указано в каждом сообщении. Слот (`morning`/`midday`/`evening`)
  определяется по тому, какое из трёх cron-расписаний сработало (`github.event.schedule`),
  а не по текущему часу — так порядок и время слотов можно менять, не боясь путаницы.
